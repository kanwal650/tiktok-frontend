<template>
    <div 
        id="SideNavMain" 
        :class="['/', '/following'].includes(route.fullPath) || route.fullPath.split('/').includes('search') ? 'lg:w-[310px]' : 'lg:w-[220px]'" 
        class="fixed z-20 bg-white pt-[70px] h-full lg:border-r-0 border-r w-[75px] overflow-auto"
    >
        
        <div class="lg:w-full w-[55px] mx-auto">
            <NuxtLink to="/">
                <MenuItem iconString="For You" :colorString="route.fullPath === '/' ? '#F02C56' : '#000000'" sizeString="30"/>
            </NuxtLink>
            <NuxtLink to="/following">
                <MenuItem iconString="Following" :colorString="route.fullPath === '/following' ? '#F02C56' : '#000000'" sizeString="27"/>
            </NuxtLink>
            <MenuItem iconString="LIVE" :colorString="route.fullPath === '/live' ? '#F02C56' : '#000000'" sizeString="27"/>
            ...
            <div v-if="$userStore.id">
                ...
                <div
                    v-if="$userStore.followings" 
                    v-for="fol in $userStore.followings"
                >
                    <div @click="isLoggedIn(fol.user)" class="cursor-pointer">
                        <MenuItemFollow :user="fol.user" />
                    </div>
                </div>
