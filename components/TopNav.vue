<template>
    ...
    >
        <div 
            :class="['/', '/following'].includes(route.fullPath) || route.fullPath.split('/').includes('search') ? 'max-w-[1150px]' : ''" 
            class="flex items-center justify-between w-full px-6 mx-auto"
        >
            <div :class="['/', '/following'].includes(route.fullPath) || route.fullPath.split('/').includes('search') ? 'w-[80%]' : 'lg:w-[20%] w-[70%]'">
                <NuxtLink to="/">
...
const search = async () => {    
    try {
        setTimeout(() => router.push(`/search/${comment.value}`), 200)
    } catch (error) {
        console.log(error)
    }
}
