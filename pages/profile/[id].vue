    ...
    <MainLayout>
        ...
        >
            ...
            <div class="flex items-center pt-4">
                <div class="mr-4">
                    <span class="font-bold">{{ $profileStore.followings.length }}</span>
                    <span class="text-gray-500 font-light text-[15px] pl-1.5">Following</span>
                </div>
                <div class="mr-4">
                    <span class="font-bold">{{ $profileStore.followers.length }}</span>
...
const isFollowed = computed(() => {
    let res = $userStore.followings.find(following => following.followee_id === $profileStore.id)
    if (res) {
        return true
    }
    return false
